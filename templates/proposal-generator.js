...
class ProposalGenerator {
    constructor() {
        ...
        this.discounts = {
            volume: {
                "3_sites": { threshold: 3, discount: 0.10, description: "10% off for 3+ websites" },
                "5_sites": { threshold: 5, discount: 0.15, description: "15% off for 5+ websites" },
                "10_sites": { threshold: 10, discount: 0.20, description: "20% off for 10+ websites" }
            },
            seasonal: {
                "new_year": { discount: 0.15, description: "New Year Special - 15% off", validUntil: "2025-01-31" },
                "summer": { discount: 0.10, description: "Summer Promo - 10% off", validUntil: "2025-08-31" }
            },
            loyalty: {
                "returning_client": { discount: 0.12, description: "Returning Client - 12% off" },
                "referral": { discount: 0.10, description: "Referral Discount - 10% off" }
            }
        };
    }
    ...
    generateProposal(clientData, customizations = {}) {
        ...
        // Calculate pricing
        proposal.pricing = this.calculatePricing(proposal, customizations.discounts);
    ...
    generateTimeline(tier, customizations = {}) {
        const baseTimelines = {
            detective: {
                totalDays: 2,
                milestones: [
                    { name: "Project Setup", day: 0, description: "Onboarding and access setup" },
                    { name: "Automated Audit", day: 1, description: "Comprehensive tracking analysis" },
                    { name: "Report Delivery", day: 2, description: "Detailed report and consultation scheduling" }
                ]
            },
            surgeon: {
                totalDays: 7,
                milestones: [
                    { name: "Project Kickoff", day: 1, description: "Alignment call and access setup" },
                    { name: "Audit Complete", day: 2, description: "Issues identified and prioritized" },
                    { name: "Implementation Start", day: 3, description: "Critical fixes begin" },
                    { name: "E-commerce Setup", day: 5, description: "Event tracking implementation" },
                    { name: "Testing & Verification", day: 6, description: "100% success validation" },
                    { name: "Training & Handoff", day: 7, description: "Team training and project completion" }
                ]
            },
            architect: {
                totalDays: 21,
                milestones: [
                    { name: "Discovery & Planning", day: 3, description: "Requirements and architecture design" },
                    { name: "Architecture Review", day: 7, description: "Stakeholder approval of design" },
                    { name: "Implementation Phase 1", day: 14, description: "Core infrastructure setup" },
                    { name: "Implementation Phase 2", day: 18, description: "Advanced features and integrations" },
                    { name: "Testing & Training", day: 20, description: "Validation and team enablement" },
                    { name: "Go Live", day: 21, description: "Production deployment and monitoring setup" }
                ]
            }
        };
        ...
        if (customizations.addOns && customizations.addOns.includes('rush_delivery')) {
    ...
    generateTerms(tier) {
        ...
        // Tier-specific terms
        if (tier === 'architect') {
            baseTerms.payment.structure = "33% upfront, 33% at midpoint, 34% on completion";
    ...
    generateProposalDocument(proposal) {
        ...
        <body>
            ...
            <div class="section">
                ...
                ${proposal.addOns.map(addOn => this.renderAddOnSection(addOn)).join('')}
            </div>
            ` : ''}
            ...
            <div class="section">
                ...
                <div class="pricing-table">
                    ...
                    <div class="total-price">
                        ...
                        ${proposal.pricing.recurringMonthly > 0 ? `<br><small>+ $${proposal.pricing.recurringMonthly}/month ongoing</small>` : ''}
                    </div>
                </div>
            </div>
            ...
            <div class="section">
                ...
                <div class="timeline">
                    <p><strong>Start Date:</strong> ${proposal.timeline.startDate}</p>
                    <p><strong>Completion:</strong> ${proposal.timeline.endDate} (${proposal.timeline.totalDays} days)</p>
                    
                    <h3>Key Milestones:</h3>
                    ${proposal.timeline.milestones.map(milestone => `
                        <div class="milestone">
            ...
            <div class="section guarantee">
                ...
                <ul>
                    ${proposal.terms.guarantees.map(guarantee => `<li>${guarantee}</li>`).join('')}
                </ul>
            </div>
            ...
            <div class="section">
                ...
                <div style="text-align: center; margin: 2rem 0;">
                    <a href="${this.generateAcceptanceUrl(proposal.id)}" class="cta-button">
            ...
            <div class="section">
                ...
                <div>
                    ...
                    A: Our ${proposal.services[0].name === 'Tracking Detective' ? '30-day' : '30-day'} support period covers any issues that arise post-implementation.</p>
                </div>
            </div>
    ...
    renderServiceSection(service) {
        return `
        <div class="pricing-table">
            <h3>${service.name} - $${service.adjustedPrice ? service.adjustedPrice.toLocaleString() : service.basePrice.toLocaleString()}</h3>
            ...
            <div class="price-item">
                <span>Additional Websites (${service.priceBreakdown.additionalSites})</span>
            ...
            <ul class="feature-list">
                ${service.customFeatures.map(feature => `<li>${feature}</li>`).join('')}
            </ul>
            ` : ''}
        </div>
        `;
    ...
    renderAddOnSection(addOn) {
        ...
        <div style="background: #f8f9fa; padding: 1rem; border-radius: 5px; margin: 1rem 0;">
            <h4>${addOn.name} - $${addOn.price.toLocaleString()}${addOn.recurring ? '/month' : ''}</h4>
