...
class LiveDashboard {
  ...
  async authenticate() {
    ...
    const response = await this.apiCall(
      "/api/protected/dashboard/" + this.clientId
    );
    if (!response.ok) {
  ...
  async loadInitialData() {
    ...
    try {
      const [dashboard, timeline, communications] = await Promise.all([
        this.apiCall("/api/protected/dashboard/" + this.clientId).then((r) =>
          r.json()
        ),
        this.apiCall("/api/protected/timeline/" + this.projectId).then((r) =>
          r.json()
        ),
        this.apiCall("/api/protected/communications/" + this.projectId).then(
          (r) => r.json()
        ),
      ]);
    ...
    switch (data.type) {
      ...
      case "step_update":
        ...
        this.showNotification(
          "Step Completed",
          `${data.step.name} has been completed`,
          "success"
        );
        break;
      ...
      case "team_response":
        ...
        this.showNotification(
          "Team Response",
          "Your team has responded to your message",
          "info"
        );
        break;
  ...
  updateAuditResults(results) {
    ...
    if (metricsCards.length >= 4) {
      ...
      metricsCards[0].querySelector(".metric-value").textContent =
      ...
      // Update issues found
      metricsCards[1].querySelector(".metric-value").textContent =
        results.totalIssues;

      // Update critical issues
      metricsCards[2].querySelector(".metric-value").textContent =
  ...
  renderDashboard(data) {
    ...
    if (statusBadge) {
      statusBadge.textContent =
        data.activeProject.status.charAt(0).toUpperCase() +
  ...
  async downloadReport() {
    try {
      const response = await this.apiCall(
        "/api/protected/download/report/" + this.projectId
      );
