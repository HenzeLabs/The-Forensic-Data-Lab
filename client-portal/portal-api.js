...
class PortalAPI {
  ...
  setupRoutes() {
    ...
    this.app.get(
      "/api/protected/dashboard/:clientId",
      this.getDashboard.bind(this)
    );
    this.app.get(
      "/api/protected/project/:projectId",
      this.getProject.bind(this)
    );
    this.app.get(
      "/api/protected/reports/:projectId",
      this.getReports.bind(this)
    );
    this.app.get(
      "/api/protected/timeline/:projectId",
      this.getTimeline.bind(this)
    );
    this.app.post("/api/protected/message", this.sendMessage.bind(this));
    this.app.get(
      "/api/protected/communications/:projectId",
      this.getCommunications.bind(this)
    );

    // Real-time audit endpoints
    this.app.post("/api/protected/audit/start", this.startLiveAudit.bind(this));
    this.app.get(
      "/api/protected/audit/status/:auditId",
      this.getAuditStatus.bind(this)
    );
    this.app.get(
      "/api/protected/audit/results/:auditId",
      this.getAuditResults.bind(this)
    );

    // File downloads
    this.app.get(
      "/api/protected/download/report/:projectId",
      this.downloadReport.bind(this)
    );
    this.app.get(
      "/api/protected/download/guide/:projectId",
      this.downloadGuide.bind(this)
    );
    ...
    this.app.post(
      "/api/admin/project/:projectId/update",
      this.updateProject.bind(this)
    );
    this.app.post(
      "/api/admin/client/:clientId/notify",
      this.notifyClient.bind(this)
    );
  }
  ...
  async downloadReport(req, res) {
    ...
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="Forensic Data Lab-Report-${projectId}.pdf"`
      );
      res.send(report);
    } catch (error) {
  ...
  getQuickActions(project) {
    return [
      { id: "download_report", label: "Download Report", icon: "report" },
      { id: "view_live_site", label: "View Live Site", icon: "search" },
      {
        id: "implementation_guide",
        label: "Implementation Guide",
        icon: "guide",
      },
      { id: "schedule_call", label: "Schedule Call", icon: "calendar" },
    ];
  }
  ...
  async start() {
    this.app.listen(this.port, () => {
      console.log(
        `The Forensic Data Lab Portal API running on port ${this.port}`
      );
      console.log(`WebSocket server running on port ${this.wsPort}`);
