...
class NotificationSystem extends EventEmitter {
  ...
  subscribe(clientId, projectId, preferences = {}) {
    ...
    console.log(
      `Client ${clientId} subscribed to notifications for project ${projectId}`
    );
    return subscription;
  }
    ...
    await this.sendNotification({
      ...
      data: {
        ...
        next_step_description:
          nextStep?.description || "Final review and handoff",
  ...
  async handleAuditCompleted(data) {
    ...
    await this.sendNotification({
      ...
      data: {
        ...
        critical_issues: auditResults.issues.filter(
          (i) => i.severity === "critical"
        ).length,
        high_issues: auditResults.issues.filter((i) => i.severity === "high")
          .length,
        medium_issues: auditResults.issues.filter(
          (i) => i.severity === "medium"
        ).length,
        estimated_monthly_loss:
          auditResults.businessImpact?.monthlyLoss || "TBD",
        header_color:
          urgency === "high"
            ? "#dc2626"
            : urgency === "medium"
  ...
  async sendNotification(notification) {
    const { type, recipients, data, priority, channels, immediate } =
      notification;
  ...
  async deliverMessage(message) {
    console.log(
      `Delivering ${message.type} notification to ${message.recipientId}`
    );
  ...
  async sendWebPush(message) {
    ...
    console.log(
      `Web push sent: ${pushPayload.title} to ${message.recipientId}`
    );
